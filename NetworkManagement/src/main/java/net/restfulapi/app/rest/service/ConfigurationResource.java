...
/**
 * This REST resource has common path "/configurations" and represents
 * configurations collection resource as well as individual collection
 * resources.
 * 
...
public class ConfigurationResource {
	...
	/**
	 * Get configurations collection resource mapped at path "HTTP GET
	 * /configurations"
	 */
	@GET
	...
	public String buildJSON(@PathParam("id") Integer id) {
		...
		Gson gson = new Gson();
		log.info("Returning: " + gson.toJson(sampleObject));
		System.out.println(gson.toJson(sampleObject));
		return gson.toJson(sampleObject);

	}

	/**
	 * Get individual configuration resource mapped at path "HTTP GET
	 * /configurations/{id}"
	 */
	...
	public Response getConfigurationById(@PathParam("id") Integer id) {
		...
		if (config != null) {
			UriBuilder builder = UriBuilder.fromResource(ConfigurationResource.class).path(ConfigurationResource.class,
	...
	/**
	 * Modify EXISTING configuration resource by it's "id" at path
	 * "/configurations/{id}"
	 */
	@PUT
	@Path("/{id}")
	...
	public Response updateConfiguration(@PathParam("id") Integer id, Configuration config) {
		...
		return Response.status(javax.ws.rs.core.Response.Status.OK).entity(new Message("Config Updated Successfully"))
				.build();
	}
	...
	/**
	 * Initialize the application with these two default configurations the
	 * following adds three entries to the stack.
	 */

	static {
		log.info("creating initial configs for testing or whatever is needed.");
